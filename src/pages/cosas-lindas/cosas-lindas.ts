...
export class CosasLindasPage {
  ...
  ionViewDidEnter(){
    this.coleccionTipadaFirebase= this.objFirebase.collection<usuario>('RelevamientoVisual'); 
    ...
    public crearDocumentoUsuario(user){
      ...
      this.objFirebase.collection<usuario>('RelevamientoVisual').doc(user).set(objetoJsonGenerico).then(
       Retorno=>
       {
  ...
  takePicture() {
    this.camera.getPicture(this.cameraOptions)
      .then(data => {
        ...
        if (imgName != "4") {
          this.imageSrv.uploadImage(base64Image, this.usuario+"cl",imgName);
          this.downloadImageUrls();
          this.updateUsuario();
        }
        else{
  ...
  downloadImageUrls() {
    ...
    this.ListaDeUsuarios.forEach(usu => {
      ...
      for (let i = 0; i < this.imagesLenght ; i++) {
        let aux = String(i+1);
        console.log ("aux = "+aux);
        let promise = this.imageSrv.getImage(usu.usuario+"cl", ""+aux+".jpg");
